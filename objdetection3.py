...
class ObjDetection3:
        ...
        for row in range(train_row):
            for col in range(train_col):
                ...
                if np.max(edges)==255:
                    ...
                    #train_set[:,row*train_row+col] = grid_img.reshape(grid_size*grid_size)
                    train_set.append(grid_img.flatten())
                    pos_row_set.append(row)
                    pos_col_set.append(col)
        #print(f"Input shape: {imgInput.shape}, {train_row}, {train_col}, output size: {len(train_set)}")

        return np.array(train_set).T, pos_row_set, pos_col_set
    ...
    def recognise_imagette2(self, imgInput, pos_row, pos_col, imgMean, W, U, nnLearner):

        imgDistThreshold = self.imgDistThreshold
        weightDistThreshold = self.weightDistThreshold
        nnDistThreshold = self.nnDistThreshold
        eigen_vector_used = self.eigen_vector_used
        dfPosIdx = self.dfPosIdx
        ...
        #print("dfPosIdx:")
        candidateWeights = dfPosIdx.loc[(dfPosIdx['pos_row'] == pos_row) & (dfPosIdx['pos_col'] == pos_col)]

        minIdx = -1
        minDist = 999999999
        for i in range(len(candidateWeights.index)):
            curIdx = dfPosIdx.index.get_loc(candidateWeights.index[i])
            curDist = np.linalg.norm(W_input[0:10] - W[:,curIdx])
            #print("Current Idx=",curIdx, "Distance =", curDist)
            if curDist < minDist:
                minDist = curDist
                minIdx = curIdx
        objIdx = dfPosIdx.iloc[minIdx]['objIdx_train']
        return objIdx
    ...
    def recognise_rgb2(self, imgInput):
        matchedCount = 0
        t0 = time.time()
        imagettes, pos_row_set, pos_col_set = self.slice_image_to_imagettes_rgb(imgInput)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = []
            for i in range(imagettes.shape[1]):
                futures.append(executor.submit(self.recognise_imagette2, imagettes[:,i], pos_row_set[i], pos_col_set[i], self.imgMean_train,
    ...
    def train_all(self, testRecognition=False, exportWeights=False):
        ...
        for trainImgFile in trainImgFiles:
            ...
            if imgTrain is not None:
                ...
                new_train_set, new_pos_row_set, new_pos_col_set = self.slice_image_to_imagettes_rgb(imgTrain)
                ...
                pos_row_set.extend(new_pos_row_set)
                pos_col_set.extend(new_pos_col_set)
        ...
        dfTrainSet = pd.DataFrame()
        dfTrainSet['pos_row'] = pos_row_set
        dfTrainSet['pos_col'] = pos_col_set
        #dfTrainSet['W_train'] = dfTrainSet.apply(W, axis=1)
        dfTrainSet['objIdx_train'] = objIdx

        self.dfPosIdx = dfTrainSet
...
def main():
    objDet = ObjDetection3()
    #objDet.load_hist()
    #return 0

    objDet.train_all(testRecognition=True, exportWeights=True)
